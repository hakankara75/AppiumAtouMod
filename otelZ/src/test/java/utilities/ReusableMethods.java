...
public class ReusableMethods {
  ...
  * @param duration yerine int cinsinden saniye olarak sure koyulur
  ...
  public static void dragDrop(AndroidDriver driver, WebElement element, int endX, int endY){
    ...
    Driver.getDriver().executeScript("mobile: dragGesture", ImmutableMap.of(
            "elementId", ((RemoteWebElement) element).getId(),
            "endX", endX,
            "endY", endY,
            "speed", 500
  ...
  public static void scrollToElementWithDirection(AndroidDriver driver,WebElement element, String direction, double x){
    driver.executeScript("mobile: scrollGesture", ImmutableMap.of(
            "elementId", ((RemoteWebElement) element).getId(),
            "direction", direction,
            "percent",x,
   ...
   */
  public static String getToastMessage(String locate) {
    String toastMessage= Driver.getDriver().findElement(By.xpath(locate)).getAttribute("name");
    return toastMessage;
  }

  /**
   * Bu metot ile normal yollardan click yapılamayan elemente click yapılır
   * @param element yerine click yapılmak istenen elementin locate verilir
   */
  public void clickByNonClickableElement(WebElement element) {
    Point location = element.getLocation();
    Dimension size = element.getSize();
    int x = location.getX() + size.getWidth() / 2;
    int y = location.getY() + size.getHeight() / 2;
    JavascriptExecutor js = (JavascriptExecutor) Driver.getDriver();
    js.executeScript("mobile: tap", new HashMap<String, Object>() {{
      put("x", x);
      put("y", y);
    }});
  }
